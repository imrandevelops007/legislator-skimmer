...
def analyze_activity_item(client, url, legislator_name):
    inferred_bill = extract_bill_from_url(url)
    
    prompt = f"""
    Analyze the following Michigan legislative activity item for legislator {legislator_name}:
    URL: {url}
    Inferred Bill Number: {inferred_bill}
    
    Extract or provide:
    1. "bill_number": (e.g., HB 4001, SB 0357)
    2. "bill_title": Concise descriptive title for this bill topic.
    3. "bill_summary": Concise 2-sentence summary of the bill's objectives.
...
def main():
    ...
    for idx, row in enumerate(rows):
        ...
        if str(processed_flag).upper() == "TRUE":
            continue
        ...
        try:
            ...
            err_str = str(e)
            if "404" in err_str or "NOT_FOUND" in err_str or "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                print("Stopping analysis loop immediately due to model availability or quota limits.")
                break
